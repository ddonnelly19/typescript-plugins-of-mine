...
/**
 * Finds a child node within a given parent node that satisfies the specified predicate.
 *
 * @param parent - The parent `ts.Node` to search within. If `undefined`, the function returns `undefined`.
 * @param predicate - A function that takes a `ts.Node` as input and returns `true` if the node matches the desired condition.
 * @param recursive - A boolean indicating whether to search recursively through all descendant nodes (`true`) 
 * or only through the immediate children of the parent node (`false`). Defaults to `true`.
 * @returns The first `ts.Node` that satisfies the predicate, or `undefined` if no matching node is found.
 */
...
export function findAscendant<T extends ts.Node>(node: ts.Node | undefined, predicate: (node: ts.Node) => boolean, orItSelf: boolean = false): T | undefined {
	...
	return ts.findAncestor(node, predicate) as T | undefined;
